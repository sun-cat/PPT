...
export function extractImageCandidate(payload) {
  ...
  if (/^https?:\/\//i.test(normalizedCandidate) || /^data:image\//i.test(normalizedCandidate)) {
    return normalizedCandidate;
  }
  return decodeBase64Image(normalizedCandidate, payload?.mime_type || "image/png");
}
...
function normalizeApiMartBase(endpoint) {
  const url = new URL(normalizeImageEndpoint(endpoint));
  url.pathname = url.pathname.replace(/\/images\/generations$/i, "");
  url.search = "";
  url.hash = "";
  return url.toString().replace(/\/$/, "");
}

function apiMartTaskEndpoint(endpoint, taskId) {
  return `${normalizeApiMartBase(endpoint)}/tasks/${encodeURIComponent(taskId)}`;
}

function apiMartUploadEndpoint(endpoint) {
  return `${normalizeApiMartBase(endpoint)}/uploads/images`;
}

function apiMartAspectRatio(value) {
  const requested = String(value || "").trim().toLowerCase();
  if (/^(?:auto|1:1|3:2|2:3|4:3|3:4|5:4|4:5|16:9|9:16|2:1|1:2|3:1|1:3|21:9|9:21)$/.test(requested)) {
    return requested;
  }
  return "16:9";
}

function apiMartResolution(value) {
  const requested = String(value || "").trim().toLowerCase();
  return /^(?:1k|2k|4k)$/.test(requested) ? requested : "1k";
}

function apiMartModel(existingTask, requestedModel) {
  const stored = String(existingTask?.productId || "").replace(/^apimart:/i, "");
  return stored || String(requestedModel || "").trim() || APIMART_DEFAULT_MODEL;
}

function markApiMartTaskPending(error, taskId, model, { final = false } = {}) {
  error.wukongTaskPending = !final;
  error.wukongTaskFinal = final;
  error.wukongTaskId = taskId;
  error.wukongProductId = `apimart:${model}`;
  return error;
}
...
async function generateApiMartImage(
  ...
  const selectedModel = apiMartModel(existingTask, model);
  ...
  const payload = {
    ...
    prompt,
    n: 1,
    size: apiMartAspectRatio(size),
    resolution: apiMartResolution(configuredResolution),
  };
  if (/-official$/i.test(selectedModel) && quality) payload.quality = quality;
  else delete payload.quality;

  let taskId = String(existingTask?.taskId || "").trim();
  if (!taskId && referenceImageDataUrl) {
    const form = new FormData();
    ...
    form.append("file", blob, filename);
    const uploadHeaders = { accept: "application/json", ...extraHeaders, ...authorization };
    for (const key of Object.keys(uploadHeaders)) {
      if (key.toLowerCase() === "content-type") delete uploadHeaders[key];
    }
    const uploadResponse = await routedFetch(apiMartUploadEndpoint(endpoint), {
      method: "POST",
      headers: uploadHeaders,
      body: form,
      signal,
    });
    const uploaded = await responseToJson(uploadResponse, "APIMart 参考图上传");
    const uploadedUrl = uploaded?.url || uploaded?.data?.url;
    if (!/^https?:\/\//i.test(String(uploadedUrl || ""))) {
      throw new Error("参考图上传成功，但 APIMart 没有返回可访问的图片 URL。");
    }
    payload.image_urls = [uploadedUrl];
  }

  if (!taskId) {
    const submitResponse = await routedFetch(normalizeImageEndpoint(endpoint), {
      method: "POST",
      headers: jsonHeaders,
      body: JSON.stringify(payload),
      signal,
    });
    const submitted = await responseToJson(submitResponse, "APIMart 生图任务提交");
    const immediateCandidate = extractImageCandidate(submitted);
    if (immediateCandidate) {
      return /^https?:\/\//i.test(immediateCandidate)
        ? remoteImageToDataUrl(immediateCandidate, routedFetch, signal)
    ...
    taskId = String(
      submitted?.data?.[0]?.task_id ||
        submitted?.data?.task_id ||
        submitted?.task_id ||
        "",
    ).trim();
    if (!taskId) throw new Error("APIMart 已响应，但没有返回生图任务 ID。");
    if (typeof onTaskSubmitted === "function") {
      await onTaskSubmitted({ taskId, productId: `apimart:${selectedModel}` });
    }
  }
  ...
  while (!signal.aborted) {
    try {
      await abortableDelay(pollIntervalMs, signal);
      const pollResponse = await routedFetch(apiMartTaskEndpoint(endpoint, taskId), {
        method: "GET",
        headers: jsonHeaders,
        signal,
      });
      const polled = await responseToJson(pollResponse, "APIMart 生图任务查询");
      ...
      const status = String(
        polled?.data?.status || polled?.status || polled?.data?.[0]?.status || "",
      ).toLowerCase();
      if (["processing", "pending", "queued", "submitted", "running"].includes(status)) {
        continue;
      }
      const detail = polled?.detail || polled?.message || polled?.error?.message;
      if (["failed", "error", "cancelled", "canceled"].includes(status)) {
        throw markApiMartTaskPending(
          new Error(`APIMart 生图任务失败：${detail || "上游没有返回失败详情。"}`),
          taskId,
          selectedModel,
          { final: true },
        );
      ...
      throw markApiMartTaskPending(
        new Error(
          `APIMart 生图任务返回了无法识别的状态${status ? `（${status}）` : ""}：${detail || "没有图片 URL。"}`,
        ),
        taskId,
        selectedModel,
      );
    } catch (error) {
      if (error?.wukongTaskFinal || error?.wukongTaskPending) throw error;
      if (isNetworkError(error) && consecutiveNetworkFailures < 5) {
        consecutiveNetworkFailures += 1;
        continue;
      }
      throw markApiMartTaskPending(error, taskId, selectedModel);
    }
  }

  const abortError = new Error("APIMart 生图任务已取消。");
  abortError.name = "AbortError";
  throw markApiMartTaskPending(abortError, taskId, selectedModel);
}
...
export async function testImageApiConnection(
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (isApiMartEndpoint(endpoint)) {
        const selectedModel = String(model || "").trim() || APIMART_DEFAULT_MODEL;
        return {
          ok: true,
          verified: true,
          status: response.status,
          ...
          adjustedEndpoint: normalizeApiMartBase(endpoint),
          adjustedModel: selectedModel,
          adjustedSize: "16:9",
          message: `APIMart 地址与 Key 鉴权通过，已配置 ${selectedModel}；生成时会自动提交异步任务并取回图片。`,
        };
      }
