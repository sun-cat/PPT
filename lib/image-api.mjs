...
const WUKONG_DEFAULT_PRODUCT = "image_gptImage2";
...
async function generateWukongStudioImage(
  ...
  { routedFetch, signal, pollIntervalMs },
) {
  const configuredProduct = String(extraBody.product_id || model || "").trim();
  ...
  if (productId === WUKONG_DEFAULT_PRODUCT) {
  ...
  while (!signal.aborted) {
    ...
    if (["failed", "error", "cancelled", "canceled"].includes(status)) {
      throw new Error(`悟空生图任务失败：${detail || "上游没有返回失败详情。"}`);
    }
    throw new Error(
  ...
  try {
    if (isWukongStudioEndpoint(endpoint)) {
      return await generateWukongStudioImage(
        ...
        { routedFetch, signal: controller.signal, pollIntervalMs },
      );
    }
    ...
    if (error?.name === "AbortError") {
      throw new Error(`图片生成超时（${Math.round(timeoutMs / 1000)} 秒）。`);
    }
